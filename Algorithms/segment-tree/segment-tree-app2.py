...
Sum, searching array prefix with a given value
...
#find the searching array prefix with a given value
def find(v, l, r, val):
    ans = None
    if val > tree[v]: ans = -1
    elif l == r: ans = l
        ...
        m = l + ((r - l) >> 1)
        if tree[v + 1] >= val: ans = find(v + 1, l, m, val)
        else: ans = find(v + 2 * (m - l + 1), m + 1, r, val - tree[v + 1])
    return ans
